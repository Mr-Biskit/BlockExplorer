...
import { getEthPrice, getMarketCap } from "../lib/coinGecko";
import { getLatestBlockNumber, getGasPrice } from "../lib/alchemySDK";
import { BigNumber } from "../typings";
import { formatBigNumberToGwei } from "../lib/conversions";

export default async function StatCard() {
  const price = await getEthPrice();
  const marketCap = await getMarketCap();
  const latestBlockNumber = await getLatestBlockNumber();
  const bigNumber: BigNumber = await getGasPrice();
  const gasPrice = formatBigNumberToGwei(bigNumber);
  ...
  cursor-pointer "
        ...
        <div className="text-right">
          <p className="text-l">${marketCap}</p>
  ...
  cursor-pointer "
        ...
        <div className="text-right">
          <p className="text-l">{gasPrice} gwei</p>
  ...
  cursor-pointer "
        ...
        <div className="text-right">
          <p className="text-2xl">{latestBlockNumber}</p>
          <p>Total Blocks</p>
        </div>
      </div>
    </div>
